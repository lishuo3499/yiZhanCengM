...
//        设置缓存
        ...
        $user_cache_data = Cache::get('user');
        $user_yb_id = $user_cache_data["yb_userid"];
//        查询用户数据是否在数据库
        $user_exist = Db::table('user')->where('yb_userid', $user_yb_id)->select();
        if (count($user_exist) == 0) {
            Db::table('user')->data([
                'yb_userid' => $user_cache_data["yb_userid"],
                'yb_realname' => $user_cache_data["yb_realname"],
                'yb_schoolid' => $user_cache_data["yb_schoolid"],
                'yb_schoolname' => $user_cache_data["yb_schoolname"],
                'yb_collegename' => $user_cache_data["yb_collegename"],
                'yb_classname' => $user_cache_data["yb_classname"],
                'yb_enteryear' => $user_cache_data["yb_enteryear"],
                'yb_studentid' => $user_cache_data["yb_studentid"],
                'yb_examid' => $user_cache_data["yb_examid"],
                'yb_admissionid' => $user_cache_data["yb_admissionid"],
                'yb_employid' => $user_cache_data["yb_employid"]])->insert();
             $this->redirect('http://127.0.0.1/yiZhanCengM/public/index');
        } else {
            dump($user_exist[0]['id']);
            $user_score = Db::table('user')->where('yb_userid',$user_yb_id)->find();
